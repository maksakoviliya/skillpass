  ...
  <div>
    <div class="lg:hidden">
      ...
      >
        ...
        <div class="mb-3.5 flex items-center">
          <span class="text-gray-500">{{ pickColumns(row.columns)[0].label }}: </span>&nbsp;
          <visual-bar class="w-full" :value="pickColumns(row.columns)[0].value"
                      :color="pickColumns(row.columns)[0].color ?? undefined"
                      :bg-color="pickColumns(row.columns)[0].bgColor ?? undefined"></visual-bar>
        </div>
      </div>
    </div>
    <div class="hidden lg:block">
      <table class="w-full">
        ...
        <tr>
          <th class="border-r border-indigo-300 last:border-0 px-2.5 first:pl-0 text-left pb-3 text-sm leading-snug font-normal"
              v-for="heading in rows[0].columns.map(item => {
            return {
              label: item.label, key: item.key
            }
          })" :key="heading.key">{{ heading.label }}
          </th>
        </tr>
        </thead>
        <tbody>
        <tr v-for="row in rowsComputes" :key="row.id">
          <td class="border-r border-indigo-300 last:border-0 px-2.5 first:pl-0 text-left text-sm py-3 leading-snug font-normal"
            ...
            <template v-if="column.key === 'visual'">
              <visual-bar class="w-full" :value="column.value"
                          :color="column.color ?? undefined"
                          :bg-color="column.bgColor ?? undefined"></visual-bar>
            </template>
            <template v-else>
              {{ column.value }}
