<template>
  ...
  >
    ...
    <div
        class="fixed inset-0 bg-gray-700 bg-opacity-80 flex-col items-center justify-center p-6 z-40"
        @click="showExitForm = false"
        :class="[
              showExitForm ? 'flex' : 'hidden',
              wideMenu ? 'lg:left-57.5' : 'lg:left-25',
            ]"
    >
      <div class="h-235 px-10 lg:px-25 bg-white rounded-2xl w-full max-w-500 flex flex-col items-center justify-center relative">
        <button class="absolute top-6 right-6 hover:text-violet-800" @click="showExitForm = false">
          <svg fill="none" class="stroke-current w-3.5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16">
            <path d="M1 15 15 1M15 15 1 1" stroke-width="2" stroke-linecap="round"/>
          </svg>
        </button>
        <h4 class="text-15 leading-snug">Вы уверены, что хотите выйти из программы?</h4>
        <div class="flex justify-between mt-10 gap-7.5">
          <button
              class="h-9 w-25 lg:w-32 bg-indigo-200 border border-indigo-200 hover:bg-violet-800 hover:border-violet-800 hover:text-white rounded-xl"
              @click="showExitForm = false">Нет
          </button>
          <button class="h-9 w-25 lg:w-32 border border-indigo-200 hover:bg-violet-800 hover:text-white rounded-xl">Да
          </button>
        </div>
      </div>
    </div>
